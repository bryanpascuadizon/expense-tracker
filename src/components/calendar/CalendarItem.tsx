import { fetchUserExpensesWithDate, makeCalendar } from "@/lib/ExpenseActions";
...
const CalendarItem = (dayItem: CalendarItemProps) => {
  const { month, day, year, expenseList } = dayItem;
        ...
        <>
          {expenseList.slice(0, 3).map((expense: ExpenseType) => (
            <div className="grid grid-cols-2 text-xs">
              <p className={`text-[${expense.tag.color}]`}>- {expense.name}</p>
